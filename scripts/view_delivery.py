...
def main():
    ...
    hybrid_scene=report.get('hybrid_preference',False)
    ...
    if args.video:
        ...
        with mujoco.Renderer(model,height=608,width=960) as renderer,iio.get_writer(Path(args.path)/'fly_in_3d.mp4',fps=25) as writer:
            for i in range(len(tape['time_s'])):
                ...
                title='3D FLY | '+('recorded memory / engineered navigation | '+report['status'] if hybrid_scene else 'archived-output mechanical playback' if archived_scene else 'neural-to-speed test | '+report['status'] if neural_scene else 'physical odor-sensor test' if odor_scene else 'mechanical walking, turning and rest')
