...
def main():
    ...
    sim=HybridTurningController(fly=fly,arena=FlatTerrain(),cameras=[],timestep=.0001,seed=101)
    stream=SensoryStream(); rows=[]; rgb=[]; angles=[]; velocities=[]; loads=[]
    outcome=dict(completed=False,brain_simulated=False,
