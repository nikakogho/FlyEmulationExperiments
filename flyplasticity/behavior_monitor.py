...
class BehaviorMonitor:
    ...
    def observe(self,t,xyz,command,odor,*,stimulus_enabled=True):
        ...
        if (type(stimulus_enabled) is not bool or p.shape!=(3,) or u.shape!=(2,) or c.shape!=(2,4) or not np.isfinite(np.r_[t,p,u,c.ravel()]).all()
        ...
        self.last=t;self.rows.append((t,p.copy(),u.copy(),float(c[:,2:].max()),stimulus_enabled))
        ...
        avoidance=all(r[4] for r in self.rows) and fullspan>=.45 and onset and after_peak>=.15 and peak-stimuli[-1]>.05 and displacement>.1
